...
import { mutation, query } from "../_generated/server"; /* Query(Fetches data from database without changing it):
                                                          Mutation(Modifies or adds new data to the database) */
import { v } from "convex/values";

export const list = query({  /* This function retrieves all the messages from the messages table */
  handler: async (ctx) => {  /* It queries the messages table, collects all entries, and returns them */
    return await ctx.db.query("messages").collect();
  },
});

export const create = mutation({ /* This function adds a new message to the messages table */
  args: {   /* Takes sender (name of the person) and content(message) as arguments */
    sender: v.string(),
    content: v.string(),
